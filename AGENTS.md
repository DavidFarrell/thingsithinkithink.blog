...
Correct:
```markdown
[Part 1]({{< ref "posts/2026/01-12-exploring-Codex-agent-sdk.md" >}})
```

Rules:
- Reference the **actual filename** (with `.md`), NOT the slug. On this site the slug often differs from the filename (e.g. the Codex Agent SDK posts), and the rendered URL is slug-based - so a hand-written path built from the filename, or a guessed slug, will be wrong.
...
- **Use David's actual words and analogies.** When David dictates an outline, his phrasing, analogies (e.g. balloon animals), and sentence constructions are the content. Don't paraphrase them into more "writerly" language - his voice IS the style. Codex's job is to organise and lightly polish, not rewrite
